...
server.registerTool(
  "remember",
  {
    title: "Remember",
    description:
      "Store something durable about the user — a preference, a decision, a constraint, a fact. " +
  ...
  async ({ content, user_id, agent_id, session_id }) =>
    run(async () => {
      ...
      if (outcomes.length === 0) return "Nothing durable to remember in that.";
      return outcomes
...
server.registerTool(
  "recall",
  {
    title: "Recall",
    description:
      "Retrieve what is known about the user that bears on the current task. Call this before " +
...
server.registerTool(
  "record_experience",
  {
    ...
    description:
      "Record what happened when you attempted a task — what you did, whether it worked, and why. " +
...
server.registerTool(
  "recommend",
  {
    title: "Recommend",
    description:
      "Ask what previous attempts at a task suggest before starting it. Returns a recommendation " +
  ...
  async ({ task }) =>
    run(async () => {
      ...
      if (!result.recommendation) return "No prior experience with anything like this task.";
