...
- Anchor time. If the input carries a date or timestamp, resolve every relative reference against it and write the absolute date into the memory: "yesterday" in text dated 8 May 2023 becomes "on 7 May 2023", "last year" becomes the year, "next weekend" becomes the dates. A memory that says "yesterday" is unreadable a week later, and a date left in the source text is not recoverable once the text is gone. If the input carries no date, write the relative phrase exactly as the user put it ("moved to Lisbon last month") and invent nothing — a guessed date is a false memory.
- A memory is a statement about the user, and nothing else. Never annotate it with commentary about the input, about what you could not determine, or about your own uncertainty — no "(relative to the message date)", no "precisely unknown", no parenthetical hedging. Uncertainty belongs in the confidence field, which is what it is for. If you cannot state something cleanly as a fact about the user, extract less rather than explaining yourself inside the content.
